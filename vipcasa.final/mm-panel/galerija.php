...
<div id="nav">
<ul id="navmenu-h"> 
<li><a href="./index.php?modul=galerija&stavka=4&LID=<?php echo $LID; ?>">Lista slika</a></li>
<li><a href="./index.php?modul=galerija&stavka=1&LID=<?php echo $LID; ?>">Upload nove slike</a></li>
<li><a href="./index.php?modul=galerija&stavka=50&LID=<?php echo $LID; ?>">Raspored slika</a></li>
</ul>
</div>
</div>
...
{
$stavka = $_GET['stavka'];
if($stavka==50){ //RASPORED SLIKA
    echo "<h2>Choose gallery:</h2>";
    $upit_akcija = "SELECT id, naziv FROM lokacija";
    $result_akcija = mysql_query($upit_akcija) or die(mysql_error());;
     echo "<form action=\"index.php?modul=galerija&LID=$LID&stavka=51\" enctype=\"multipart/form-data\" method=\"POST\">";
    ...
    echo "</select><input type=\"submit\" value=\"Pogledaj\">";
}
if($stavka==51){ //rasporedjivanje
$akcija = $_POST['akcija'];

$query_smestaj  = "SELECT naziv FROM lokacija WHERE id='$akcija' ";
$result_smestaj = mysql_query($query_smestaj);
list($naziv_smestaja)=@mysql_fetch_row($result_smestaj);
?>
...
$(document).ready(function(){ 
    ...
    $(function() {
        $("#contentLeft ul").sortable({ opacity: 0.6, cursor: 'move', update: function() {
            ...
            $.post("reorder/updateDB.php", order, function(theResponse){
...
<div id="contentWrap">
        ...
        <div id="contentTop">
        <h2>Choose gallery: <?php
    echo $naziv_smestaja;
?></h2>
          <p>Misem pomerajte redove kako bi promenili redosled.</p>
      </div>
    
        <div id="contentLeft">
            <ul>
                <?php
                $query  = "SELECT * FROM galerija WHERE akcija='$akcija' ORDER BY redosled ASC";
                $result = mysql_query($query);
                
                while($row = mysql_fetch_array($result, MYSQL_ASSOC))
                {
                ?>
                    <li id="recordsArray_<?php echo $row['id_g']; ?>"><?php echo "<img hspace=\"0\" border=\"0\" src=\"/pics/thumbs/".$row['thumb'].".jpg\" alt=\"\" />"; ?></li>
                <?php } ?>
            </ul>
        </div>
...
if($stavka==1){ //Ubacivanje slike (Stavka br.1)
...
echo "<form action=\"index.php?modul=galerija&LID=$LID\" enctype=\"multipart/form-data\" method=\"POST\">";
echo "<table class=\"okvirche\" width=\"500\" border=\"0\" align=\"center\" cellspacing=0>
  <tr>
    ...
    <tr>
    <td class=\"TD_EvenNew\" width=\"250\" style=\"padding: 10px 5px 10px 10px\">Lokacija:</td>
...
if(isset($_POST['Submit']))
{
    ...
    $sqlSLIKE = "INSERT INTO galerija (akcija, datum, title, opis, thumb, img) VALUES ('".$akcija."', '".$datum_mysql."', '".$nazivSlike."', '".$opisSlike."', 'thumb_".$novi_naziv_fajla."', '".$novi_naziv_fajla."')";
//$rez = mysql_query($sqlSLIKE);
  ...
  <tr>
    ...
    if($width>=$height){
    $newwidth=152;
    $newheight=($height/$width)*152;
    }
    if($width<$height){
    $newheight=98;
    $newwidth=($height/$width)*98;
    $newwidth=$newwidth/2;
    }
    ...
    $filename = "../pics/thumbs/thumb_". $novi_naziv_fajla.".jpg";
    



    
    $filename_full = "../pics/". $novi_naziv_fajla.".jpg";
...
$brisanjeS = "SELECT g.id_g, g.datum, g.title, g.opis, g.thumb, g.img, a.naziv
                FROM galerija g, lokacija a
                WHERE g.akcija = a.id
                ORDER BY g.akcija";
    ...
    <td class=\"TD_OddRed\" style=\"padding: 10px 5px 10px 10px\"><strong>Gallery</strong></td>
...
while ($red=mysql_fetch_array($rezultat)) 
    {
        ...
        $akcija_naziv=$red["naziv"];
        
        $broj = 1;
        echo"<tr>";
    echo "<td class=\"TD_EvenNew\">$id</td>
    <td class=\"TD_EvenNew\">$datum</td>
    <td class=\"TD_EvenNew\"><input type=\"text\" id=\"naslov_slike$id\" value=\"$naslov\" ><input type=\"button\" onClick=\"var naslov_slike = \$('#naslov_slike$id').val(); \$('#azuriranje$id').load('ajax/smestaj.php',{akcija: 'azuriraj_naslov_slike', LID:'$LID', id_g:'$id', naslov_slike: naslov_slike}).fadeIn('slow');\" value=\"Ažuriraj\"><div id=\"azuriranje$id\" ></div></td>
    <td class=\"TD_EvenNew\">$opis</td>
    <td class=\"TD_EvenNew\"><img  hspace=\"0\" border=\"0\" src=\"../pics/thumbs/$thumb.jpg\" alt=\"\" /></td>
    <td class=\"TD_EvenNew\">$akcija_naziv</td>
    <td class=\"TD_EvenNew\"><a class=\"meni\" href=\"./index.php?modul=galerija&stavka=5&c=$id&LID=$LID\">Obrisi</a></td>
  </tr>";
...
$brisanjeSL = "DELETE FROM galerija WHERE id_g= '".$brojche."'";
