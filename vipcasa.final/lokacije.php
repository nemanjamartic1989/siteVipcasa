...
<?php
    ...
    $sql = "SELECT g.id, g.naziv FROM Grad g ORDER BY g.id ASC ";
    $rez = mysql_query($sql);
          ...
          <h1 class="f-38 curly-tittle"><img src="/images/curly.png">'.$naziv.'</h1>';
              ...
              if(mysql_num_rows($rez2)>0){
                while (list($id, $naziv_lokacije, $mesto, $adresa, $status, $sold, $img)=@mysql_fetch_row($rez2)) {
                  ...
                  if($id == 5){
                    echo '<a href="javascript:void(0ß)">'; 
                  }else{
                    echo '<a href="/vipcasa-lokacija='.$id.'">'; 
                  }
                  if($sold==1){ echo '<div class="rasprodato"></div>';} echo '<img class="img-responsive"src="/pics/vipcasa/'.$img.'"><div class="title-stripe"><h2>'.$naziv_lokacije.'</h2></div></a></div></div>';                 
                }
              }
          echo '</div>';
